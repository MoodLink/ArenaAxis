...
export const getRevenueOfOwner = ({ Order }) => {
  return async (req, ownerId) => {
    try {
      let data = {};
      ...
      for (const store of stores) {
        ...
        const orders = await Order.find({
          ...
          .select("cost createdAt updatedAt")
          .lean();
...
export const getRevenueOfStoreService = ({ Field, OrderDetail }) => {
  return async (storeId) => {
    try {
      ...
      for (const field of fields) {
        ...
        const fieldOrderDetails = await OrderDetail.find({
          fieldId: field._id,
        }).lean();
