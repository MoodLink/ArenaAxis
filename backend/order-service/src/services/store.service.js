...
export async function handleUpdateSportForStore(storeId, sportId, hasSport) {
  try {
    const needUpdate = await checkSportInStore(storeId, sportId, hasSport);
    if (needUpdate) {
      await updateSportForStore(storeId, sportId, hasSport);
...
async function updateSportForStore(storeId, sportId, hasSport) {
	try {
		await axios.post(
