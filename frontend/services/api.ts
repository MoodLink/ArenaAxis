...
export async function loginUser(email: string, password: string): Promise<{ token?: string; user?: any; error?: string }> {
    try {
        const response = await fetch('https://www.executexan.store/auth/user', {
            method: 'POST',
...
export async function loginAdmin(email: string, password: string): Promise<{ token?: string; user?: any; error?: string }> {
    try {
        const response = await fetch('https://www.executexan.store/auth/admin', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ email, password })
        });
...
export async function loginClient(email: string, password: string): Promise<{ token?: string; user?: any; error?: string }> {
    try {
        const response = await fetch('https://www.executexan.store/auth/client', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ email, password })
        });
...
export async function signupUser({ name, email, password, phone }: { name: string; email: string; password: string; phone?: string }): Promise<{ success: boolean; error?: string }> {
    try {
        const response = await fetch('https://www.executexan.store/users', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
export async function refreshToken(token: string): Promise<{ token?: string; error?: string }> {
    try {
        const response = await fetch('https://www.executexan.store/auth/refresh', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
export async function logout(token: string): Promise<{ success: boolean; error?: string }> {
    try {
        const response = await fetch('https://www.executexan.store/auth/logout', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ token })
        });
...
export async function getUserById(id: string): Promise<User | null> {
    try {
        const response = await fetch(`https://www.executexan.store/users/${id}`);
        if (!response.ok) {
            return null;
        }
...
export async function getUsers(page: number = 0, pageSize: number = 30): Promise<User[]> {
    try {
        const response = await fetch(`https://www.executexan.store/users?page=${page}&pageSize=${pageSize}`);
        if (!response.ok) {
            return [];
        }
        return await response.json();
...
export async function toggleUserActive(id: string): Promise<User | null> {
    try {
        const response = await fetch(`https://www.executexan.store/users/${id}/toggle_active`, {
            method: 'PUT'
        });
        if (!response.ok) {
            return null;
...
export async function getFields(filters?: {
    ...
    try {
        ...
        for (const storeId of storeIds) {
            try {
                const storeResponse = await fetch(`https://www.executexan.store/stores/${storeId}`)
                if (storeResponse.ok) {
...
export async function getFieldById(id: string): Promise<Field | null> {
    try {
        ...
        if (field.storeId) {
            try {
                const storeResponse = await fetch(`https://www.executexan.store/stores/${field.storeId}`)
                if (storeResponse.ok) {
                    const storeData = await storeResponse.json()
                    const store = storeData.data || storeData
...
export async function getSports(): Promise<Sport[]> {
    try {
        ...
        const response = await fetch('https://www.executexan.store/sports');
